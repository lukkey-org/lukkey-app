...
export const detectNetwork = (address, preferredChain = "") => {
  ...
  // Ethereum, Binance Smart Chain, Polygon, Arbitrum, Aurora, Avalanche, Optimism, Gnosis Chain, zkSync Era Mainnet, Linea, Cronos, Mantle, Ethereum Classic, EthereumPoW, Base, Boba Network, Celo
  else if (/^0x[a-fA-F0-9]{40}$/.test(normalizedAddress)) {
    return "Ethereum (ETH)/Binance (BNB)/Polygon (POL)/Arbitrum (ARB)/Aurora (AURORA)/Avalanche (AVAX)/Optimism (OP)/Gnosis Chain (xDAI)/zkSync Era Mainnet (zkSync)/Linea (Linea)/Cronos (CRO)/Mantle (Mantle)/Ethereum Classic (ETC)/EthereumPoW (ETHW)/Base (BASE)/Boba Network (BOBA)/Celo (CELO)";
  }
  ...
  // Osmosis
  else if (/^osmo1[a-zA-Z0-9]{38}$/.test(normalizedAddress)) {
    return "Osmosis (OSMO)";
  }
