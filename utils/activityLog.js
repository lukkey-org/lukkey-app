...
import { getAssetChainFullName } from "../config/assetFields";
import {
...
const cardContainsAddress = (card, address) =>
  getCardQueryAddresses(card).some((entry) =>
    areAddressesEquivalent(getAssetChainFullName(card), entry, address),
  );
          ...
          self.findIndex(
            (c) =>
              String(getAssetChainFullName(c) || "").trim().toLowerCase() ===
                String(getAssetChainFullName(crypto) || "").trim().toLowerCase() &&
              normalizeAddressForComparison(getAssetChainFullName(c), c.address) ===
                normalizeAddressForComparison(
                  getAssetChainFullName(crypto),
                  crypto.address,
                )
          )
    );
      ...
      const mapWithChainKey = (list) => {
        return list.map((tx) => {
          ...
          const chainKey = matched
            ? `${getAssetChainFullName(matched)}:${matched.address}`
            : `${chainSym}:${addr}`;
      ...
      if (ok && Array.isArray(transactions)) {
        ...
        setActivityLogPages((prev) => {
          ...
          for (const c of uniqueCryptos) {
            const key = `${getAssetChainFullName(c)}:${c.address}`;
