...
import { getPubkeySyncRequests } from "./pubkeySyncRequests";
...
const parseHardwareVersionFromPinLine = (line) => {
  const raw = String(line || "").trim();
  if (!raw.startsWith("PIN:")) return "";
  const parts = raw
    .substring(4)
    .split(",")
    .map((part) => part.trim());
  const hardwareToken = parts.find((part) => /^HW:/i.test(part));
  return hardwareToken ? hardwareToken.replace(/^HW:/i, "").trim() : "";
};
    ...
    if (pinCodeValue === receivedPin) {
      ...
      if (flag === "Y") {
        ...
        const hardwareVersion =
          parseHardwareVersionFromPinLine(verificationCodeValue) ||
          (await getSecureItem("hardwareVersion").catch(() => ""));
        const addressSyncOptions = {
        ...
        try {
          const expected = getAddressSyncRequests(
            prefixToShortName,
            addressSyncOptions,
          ).map((request) => request.syncKey);
        ...
        const addressSyncRequests = getAddressSyncRequests(
          prefixToShortName,
          addressSyncOptions,
        );
        ...
        setTimeout(async () => {
          ...
          const pubkeyMessages = getPubkeySyncRequests();
