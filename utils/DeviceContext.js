...
const buildAssetIdentityKey = (asset) => {
  const normalizedAsset = normalizeAssetConfig(asset || {});
  const chain = normalizeAssetIdentityPart(
    getAssetChainFullName(normalizedAsset) || getAssetChainShortName(normalizedAsset),
  );
  const symbol = normalizeAssetIdentityPart(
    getAssetSymbol(normalizedAsset),
  );
  const contract = normalizeAssetIdentityPart(
    normalizedAsset?.contractAddress ||
      normalizedAsset?.contract_address ||
      normalizedAsset?.tokenContractAddress,
  );
  const tokenId = normalizeAssetIdentityPart(
    normalizedAsset?.tokenId || normalizedAsset?.mint,
  );
  const coinType = normalizeAssetIdentityPart(
    getAssetType(normalizedAsset),
  );
  return `${chain}:${symbol}:${contract}:${tokenId}:${coinType}`;
};

const mergeDefaultAssetWithSaved = (defaultAsset, savedAsset) => ({
  ...(savedAsset || {}),
  ...(defaultAsset || {}),
});
...
export const CryptoProvider = ({ children }) => {
  ...
  const matchesAddressTarget = (item, queryChainShortName) => {
    ...
    const itemQueryShort = String(getAssetChainShortName(item) || "")
      .trim()
      .toUpperCase();
    if (itemQueryShort && itemQueryShort === target) return true;
    const itemShort = String(getAssetSymbol(item) || "").trim().toUpperCase();
    const itemCoinType = String(getAssetType(item) || "").trim().toLowerCase();
  ...
  const updateCryptoData = (shortName, newData) => {
    if (supportedChains.includes(shortName)) {
      setInitialAdditionalCryptos((prevCryptos) => {
        const updatedCryptos = prevCryptos.map((crypto) =>
          getAssetSymbol(crypto) === shortName ? { ...crypto, ...newData } : crypto
        );
        AsyncStorage.setItem(
          "initialAdditionalCryptos",
          JSON.stringify(updatedCryptos)
    ...
    const loadSettings = async () => {
      try {
        ...
        if (savedInitialCryptos !== null) {
          ...
          for (const savedAsset of savedList) {
            const key = buildAssetIdentityKey(savedAsset);
            if (key) savedMap.set(key, savedAsset);
          }

          const merged = [];
          const mergedKeys = new Set();
          for (const def of initialAdditionalCryptos) {
            const k = buildAssetIdentityKey(def);
            if (savedMap.has(k)) {
              merged.push(mergeDefaultAssetWithSaved(def, savedMap.get(k)));
            } else {
              merged.push(def);
            }
            mergedKeys.add(k);
          ...
          for (const s of savedList) {
            const k = buildAssetIdentityKey(s);
            if (!mergedKeys.has(k)) {
              merged.push(s);
              mergedKeys.add(k);
            }
          ...
          const normalizedMerged = merged
            .filter(
              (card) => !isHiddenInDevelopmentAsset(card) && !isRemovedAsset(card),
            )
            .map((card) => normalizeAssetConfig(card))
            .map((card) =>
              enrichLtcAddressData(
  ...
  const getChainAddrInfo = (chain, address) => {
    ...
    const byBoth = cryptoCards.find(
      (c) =>
        String(getAssetChainFullName(c) || "").trim().toLowerCase() === chainLc &&
        areAddressesEquivalent(
          chainLc,
    ...
    const byAddr = cryptoCards.find(
      (c) =>
        areAddressesEquivalent(
          String(getAssetChainFullName(c) || "").trim().toLowerCase(),
          String(c?.address || "").trim(),
          addr,
        )
    );
    ...
    const chainName = picked
      ? String(getAssetChainFullName(picked) || "").trim()
      : chainLc;
  ...
  const buildWssTargets = (cryptos = []) => {
    ...
    (Array.isArray(cryptos) ? cryptos : []).forEach((crypto) => {
      ...
      const chain = String(getAssetChainFullName(crypto) || "").trim().toLowerCase();
