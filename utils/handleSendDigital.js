...
const BTC_PATH_BY_ADDR_FORMAT = {
  legacy: "m/44'/0'/0'/0/0",
  nested_segwit: "m/49'/0'/0'/0/0",
  native_segwit: "m/84'/0'/0'/0/0",
  taproot: "m/86'/0'/0'/0/0",
};

const resolveNftSignPath = ({ chainKey, fallbackPath, selectedNFT, paymentAddress }) => {
  if (String(chainKey || "").toLowerCase() !== "bitcoin") return fallbackPath;
  const addrFormat =
    String(
      selectedNFT?.sourceAddrFormat ||
        selectedNFT?.addrFormat ||
        resolveTransportAddrFormat("bitcoin", paymentAddress) ||
        "",
    ).trim();
  return BTC_PATH_BY_ADDR_FORMAT[addrFormat] || fallbackPath;
};
...
export const handleSendDigital = async ({
  ...
  try {
    ...
    const path = resolveNftSignPath({
      chainKey,
      fallbackPath: assetOps[chainKey],
      selectedNFT,
      paymentAddress,
    });
    if (!path) {
    ...
    logFlowStep("1/7", "ACCOUNT_ID verify", "resolve nft path", {
      ...
      sourceQueryAddress: selectedNFT?.sourceQueryAddress || "",
      sourceAddrFormat: selectedNFT?.sourceAddrFormat || "",
    });
    ...
    if (chainMethod === "evm") {
      ...
      requestData = {
        ...
        ...(protocolType ? { protocol: protocolType, protocolType } : {}),
