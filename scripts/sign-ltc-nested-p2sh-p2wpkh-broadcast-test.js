#!/usr/bin/env node
/*
 * Offline helper for signing a Litecoin Nested SegWit/P2SH-P2WPKH presign payload
 * returned by /api/sign/encode_btc. It can optionally broadcast through
 * a public Litecoin node when --broadcast is passed.
 *
 * Safety:
 * - Default mode signs only and prints diagnostics; it does not broadcast.
 * - The derived P2SH-P2WPKH address must match --from.
 * - The final signed transaction is checked so scriptSig only contains redeemScript.
 */
...
const DEFAULT_FROM_ADDRESS = "MLRH33eiTQWFgHVsedFexEoMkZeRKH12B6";
const DEFAULT_PATH = "m/49'/2'/0'/0/0";
const DEFAULT_PRESIGN_PAYLOAD = {
  hex: "",
  value: [5973238, 4829050],
  inputScriptType: "p2sh-p2wpkh",
  changeScriptType: "p2sh-p2wpkh",
  receiveScriptType: "p2pkh",
  signMode: "segwit_v0",
};
...
function usage() {
  console.log(`Usage:
  LTC_WIF=<sender_wif> node scripts/sign-ltc-nested-p2sh-p2wpkh-broadcast-test.js
  LTC_MNEMONIC="<words>" node scripts/sign-ltc-nested-p2sh-p2wpkh-broadcast-test.js
  LTC_MNEMONIC="<words>" node scripts/sign-ltc-nested-p2sh-p2wpkh-broadcast-test.js --broadcast
  node scripts/sign-ltc-nested-p2sh-p2wpkh-broadcast-test.js --wif <sender_wif> --from ${DEFAULT_FROM_ADDRESS}
  node scripts/sign-ltc-nested-p2sh-p2wpkh-broadcast-test.js --mnemonic "<words>" --path "${DEFAULT_PATH}"
  node scripts/sign-ltc-nested-p2sh-p2wpkh-broadcast-test.js --wif <sender_wif> --payload '{"hex":"...","value":[...]}'

Notes:
  - This signs Litecoin Nested SegWit/P2SH-P2WPKH inputs only.
...
function base58Encode(bytes) {
  ...
  let encoded = "";
  while (num > 0n) {
    const modValue = Number(num % 58n);
    encoded = BASE58[modValue] + encoded;
    num /= 58n;
  }
...
function base58CheckEncode(payload) {
  return base58Encode(Buffer.concat([payload, hash256(payload).subarray(0, 4)]));
}
...
function p2wpkhAddress(pubkey) {
  return bech32Encode("bc", [0, ...convertBits([...hash160(pubkey)], 8, 5, true)], 1);
}

function witnessProgramFromP2wpkhAddress(address) {
  const decoded = bech32Decode(address);
  if (decoded.hrp !== "bc") throw new Error("Only BTC mainnet bc1 addresses are supported");
...
function p2shP2wpkhRedeemScript(pubkey) {
  return Buffer.concat([Buffer.from([0x00, 0x14]), hash160(pubkey)]);
}

function p2shP2wpkhAddress(pubkey) {
  return base58CheckEncode(Buffer.concat([Buffer.from([0x32]), hash160(p2shP2wpkhRedeemScript(pubkey))]));
}
...
function pushData(data) {
  ...
  if (data.length <= 0xffff) {
    ...
    prefix[0] = 0x4d;
    prefix.writeUInt16LE(data.length, 1);
    return Buffer.concat([prefix, data]);
  }
  throw new Error("pushData only supports data up to 65535 bytes");
}
...
function signP2shP2wpkh(tx, payloadValues, privateKey, pubkey) {
  ...
  const redeemScript = p2shP2wpkhRedeemScript(pubkey);
  const scriptSig = pushData(redeemScript);
  const signedInputs = tx.inputs.map((input) => ({ ...input, script: scriptSig }));
...
function inspectSignedSegwitTx(hex) {
  ...
  return {
    inputCount,
    scriptSigLengths,
    scriptSigsContainOnlyRedeemScript: scriptSigLengths.every((length) => length === 23),
  };
}
...
async function main() {
  ...
  if (!compressed) throw new Error("P2SH-P2WPKH requires a compressed public key");

  const pubkey = publicKeyFromPrivate(privateKey, true);
  const derivedAddress = p2shP2wpkhAddress(pubkey);
  ...
  const signed = signP2shP2wpkh(tx, values, privateKey, pubkey);
  const signedHex = signed.toString("hex");
  const witnessTxid = Buffer.from(hash256(signed)).reverse().toString("hex");
  const redeemScript = p2shP2wpkhRedeemScript(pubkey);
  const senderScriptSigBuffer = pushData(redeemScript);
  const senderScriptSig = senderScriptSigBuffer.toString("hex");
  const noWitnessTx = {
    ...tx,
    inputs: tx.inputs.map((input) => ({ ...input, script: senderScriptSigBuffer })),
  };
  const txid = Buffer.from(hash256(serializeBaseTx(noWitnessTx))).reverse().toString("hex");
  ...
  const result = {
    ...
    inputScriptType: payload.inputScriptType || "p2sh-p2wpkh",
    ...
    scriptSigsContainOnlyRedeemScript: inspection.scriptSigsContainOnlyRedeemScript,
    txid,
    wtxid: witnessTxid,
    signedHex,
  };

  if (!inspection.scriptSigsContainOnlyRedeemScript) {
    throw new Error(`Invalid P2SH-P2WPKH signature: scriptSig lengths are ${inspection.scriptSigLengths.join(",")}`);
  }
