...
import { getExpectedPubkeySyncKeys } from "./utils/pubkeySyncRequests";
...
function MainStackNavigator() {
  ...
  return (
    ...
    >
      ...
      <MainStack.Screen
        name="StandaloneGeneral"
        component={SecureDeviceScreen}
        options={{
          title: t("General"),
          headerShadowVisible: false,
          headerStyle: {
...
function AppContent({
  ...
  const allowDevWalletWithoutPairing = RUNTIME_DEV || __DEV__;
  const isWalletCreationSyncing = verificationStatus === "waiting";
  const isPairedOrDev =
    settingsLoaded &&
    !isWalletCreationSyncing &&
    hasCryptoCards &&
    (isPaired || allowDevWalletWithoutPairing);

  useEffect(() => {
    if (!__DEV__) return;
    const hasCards = Array.isArray(cryptoCards) && cryptoCards.length > 0;
    const pairedOrDev = isPaired || allowDevWalletWithoutPairing;
    ...
    // console.log("[PAIR_STATE] isWalletCreationSyncing:", isWalletCreationSyncing);
  ...
  const hideBottomTabBar =
    walletModalVisible || !isPairedOrDev || isWalletCreationSyncing;
    ...
    <View style={{ flex: 1, backgroundColor: bottomBackgroundColor }}>
      ...
      {isWalletCreationSyncing ? (
        ...
        <Tab.Navigator
          ...
          screenOptions={({ route }) => ({
            ...
            tabBarStyle: {
              ...
              borderTopWidth: 0,
              height: hideBottomTabBar ? 0 : 100,
              paddingBottom: hideBottomTabBar ? 0 : 30,
              borderTopLeftRadius: 22,
              borderTopRightRadius: 22,
              display: hideBottomTabBar ? "none" : "flex",
        ...
        >
          <Tab.Screen
            ...
            options={({ route, navigation }) => {
              ...
              const routeIsWalletCreationSyncing =
                isWalletCreationSyncing ||
                route.params?.isWalletCreationSyncing === true;
              return {
                headerLeft: () =>
                  routeIsWalletCreationSyncing ? null : route.params?.isModalVisible ? (
                    <TouchableOpacity
                      onPress={() => {
                ...
                headerRight: () =>
                  routeIsWalletCreationSyncing ? null : (
                    <AssetsHeaderActions
        ...
        <GetStartedScreen
          ...
          onOpenGeneral={() => navigation.navigate("StandaloneGeneral")}
        ...
        >
          <View style={{ flex: 1 }}>
            <Pressable
              ...
              onPressIn={() => setHeaderDropdownVisible(false)}
            >
      ...
      />
      {RUNTIME_DEV && !isWalletCreationSyncing && <FloatingDev />}
    </View>
  );
}
