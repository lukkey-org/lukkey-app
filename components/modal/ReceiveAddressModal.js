...
const ReceiveAddressModal = ({
  ...
  const normalizedReceiveChain = String(queryChainShortName || "")
    .trim()
    .toUpperCase();
  const isBchReceiveChain =
    normalizedReceiveChain === "BCH" ||
    normalizedReceiveChain === "BITCOIN_CASH";
  const resolvedBchCashQrAddress = isBchReceiveChain
    ? resolveBchAddressByType(
    ...
    : "";
  const qrAddress =
    isBchReceiveChain && isBchCashAddr(resolvedBchCashQrAddress)
      ? resolvedBchCashQrAddress
      : address;
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styleObj.centeredView} pointerEvents="box-none">
          ...
          >
            ...
            />
            <QRCodeView address={qrAddress} cryptoIcon={cryptoIcon} />
...
const AddressTypeToggle = ({
  ...
  const horizontalPadding = compact ? 6 : 12;
  const toggleWidth =
    labelWidth > 0 ? labelWidth + horizontalPadding * 2 + 20 : undefined;
  ...
  const totalBalanceFormatted =
    totalBalanceValue === 0
      ...
      : formatCryptoBalanceDisplay(totalBalanceValue, {
          ...
          context: "ReceiveAddressModal.addressTypeTotal",
          compactLarge: true,
        });
  const totalBalanceText = compactIfTooLong(
    totalBalanceFormatted,
    totalBalanceValue,
  );
  const totalFiatText =
    typeof getBalanceFiatText === "function"
      ? getBalanceFiatText(totalBalanceValue, balanceSymbol)
      : "";
...
const AddressInfo = ({
  ...
  let addressRows = [];
  /*
   * Temporary production restriction: BCH receive addresses use the single
   * address display. Restore this active type when legacy selection is
   * re-enabled.
   *
  ...
  const hasCashAddr = Boolean(isBchCashAddr(resolvedCashAddr));
  /*
   * Temporary production restriction: BCH receive addresses only expose
   * CashAddr. Keep the legacy resolution code commented so the previous
   * selector can be restored quickly after legacy support is re-enabled.
   *
   ...
   *   bchLegacyAddr,
   * );
   * Restore isBchLegacyAddress import before re-enabling this line:
   * const hasLegacyAddr = Boolean(isBchLegacyAddress(resolvedLegacyAddr));
  ...
  const displayCashAddr = resolvedCashAddr
    ...
    : "";
  /*
   * Temporary production restriction: BCH now uses single address display.
   * Restore this options array when legacy receive selection is re-enabled.
   *
   * const bchOptions = [
   *   {
   ...
   */
  /*
   * Restore this fallback when legacy BCH receive selection is re-enabled.
   *
   * if (isBchChain) {
   *   if (hasCashAddr) {
  ...
  if (hasValidAddress) {
    if (isBchChain && hasCashAddr) {
      ...
      /*
       * Temporary production restriction: legacy-only BCH receive display is
       * disabled. Restore this branch with hasLegacyAddr/resolvedLegacyAddr
       * when legacy support is re-enabled.
       *
       * } else if (isBchChain && hasLegacyAddr) {
       *   addressRows = [
       *     {
       *       label: "",
       *       labelNode: receiveMultiAddressEnabled ? (
       *         <AddressTypeToggle
       ...
       *           cryptoIcon={cryptoIcon}
       *           balanceSymbol="BCH"
       *           getBalanceFiatText={getTypedFiatText}
       *           totalLabel={t("Total")}
       *         />
       *       ) : null,
