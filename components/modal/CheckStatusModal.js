...
const PUBLIC_BETA_CONTACT_EMAIL = ["info", "lukkey.com"].join("@");
const PUBLIC_BETA_SOCIAL_URL = ["https://", "lukkey.com/social"].join("");
...
const CheckStatusModal = ({
    ...
    const loadVersions = async () => {
      if (
        (status === "firmwareUpdateInfo" ||
          ...
          status === "otaLatest" ||
          status === "publicBetaVersion") &&
        visible
      ) {
        try {
          const [hw, bt, base] = await Promise.all([
            getSecureItem("hardwareVersion"),
            getSecureItem("bluetoothVersion"),
            getSecureItem("baseVersion"),
          ]);
          if (!cancelled) {
            setHwVersion(hw || null);
            setBtVersion(bt || null);
            setBaseVersion(base || null);
          }
        } catch (e) {
          if (!cancelled) {
            setHwVersion(null);
  ...
  const isLegacyTransactionSuccess = status === "success" && txHash;
  const isLegacyTransactionFail = status === "fail" && txHash;

  if (isLegacyTransactionSuccess) {
  ...
  } else if (status === "txSent") {
    imageSource = SuccessGif;
    title = t("Transaction Sent");
    subtitle = t("Waiting for confirmation on the blockchain.");
  } else if (status === "txSuccess") {
    imageSource = SuccessGif;
  ...
  } else if (isLegacyTransactionFail) {
    imageSource = FailGif;
    title = t("Transaction Failed");
    subtitle = t("The transaction could not be completed. Please try again.");
  } else if (status === "fail") {
  ...
  } else if (status === "publicBetaVersion") {
    imageSource = SuccessGif;
    title = t("Your device is up to date");
    subtitle = t(
      "You are using the latest public beta version. If you have any requests, please contact {{email}} or follow {{socialUrl}} to contact the official LUKKEY team.",
      {
        email: PUBLIC_BETA_CONTACT_EMAIL,
        socialUrl: PUBLIC_BETA_SOCIAL_URL,
      },
    );
