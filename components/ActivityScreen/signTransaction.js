...
import { getLtcAddressType } from "../../utils/ltcAddress";
import { getTransactionFailureReasonKey } from "../../utils/transactionFailureReason";

const LOG_GREEN = "\x1b[32m";
const LOG_RESET = "\x1b[0m";
const LTC_PATH_BY_ADDR_FORMAT = {
  legacy: "m/44'/2'/0'/0/0",
  nested_segwit: "m/49'/2'/0'/0/0",
  native_segwit: "m/84'/2'/0'/0/0",
};
...
const parseCosmosBaseFeeAmount = (value) => {
...
const resolveCosmosFeeAmount = ({
  ...
  const backendFeeAmount = parseCosmosBaseFeeAmount(feeAmountRaw);
  if (backendFeeAmount != null) {
    return {
      feeAmount: backendFeeAmount,
      feeAmountStr: String(backendFeeAmount),
      feeSource: "backendFeeAmount",
      candidates: [{ source: "backendFeeAmount", amount: backendFeeAmount }],
    };
  }

  const candidates = [];
  const pushCandidate = (source, value) => {
    const amount = parseCosmosBaseFeeAmount(value);
    if (amount != null) candidates.push({ source, amount });
  };

  const gp = Number(gasPrice);
  const gl = Number(gasLimit);
  if (Number.isFinite(gp) && gp > 0 && Number.isFinite(gl) && gl > 0) {
    pushCandidate("gasPrice*gasLimit", gp * gl);
  }

  const selectedFeeCoin = Number(
    selectedFeeTab === "Recommended" ? recommendedFee : rapidFeeValue,
  );
  if (Number.isFinite(selectedFeeCoin) && selectedFeeCoin > 0) {
    pushCandidate("uiSelectedFee", Math.round(selectedFeeCoin * 1e6));
  }

  const best = candidates[0] || { source: "fallback", amount: 3000 };

  return {
    feeAmount: best.amount,
    feeAmountStr: String(best.amount),
    feeSource: best.source,
    candidates,
  };
};
...
const resolveTronFeeSun = (feeValue) => {
  const fee = Number(feeValue);
  if (!Number.isFinite(fee) || fee <= 0) return 1000000;

  // UI fee values below 1 are TRX-denominated. Integer values are treated as
  // SUN to preserve compatibility with the embedded TRON signer test vectors.
  if (fee < 1) {
    return Math.max(1, Math.round(fee * 1e6));
  }

  return Math.max(1, Math.round(fee));
};

const resolveTronSigningDecimals = ({ tokenDecimals }) => {
  const amountDecimals = resolvePositiveInteger(tokenDecimals, 6);

  return {
    amountDecimals,
    overrideDecimals: amountDecimals,
  };
};
...
const signTransaction = async (
  ...
  try {
    ...
    const resolveLtcPathByAddress = (addr) => {
      const addrFormat = getLtcAddressType(addr);
      return LTC_PATH_BY_ADDR_FORMAT[addrFormat] || assetOps.litecoin;
    };
    ...
    const path =
      chainKey === "bitcoin"
        ...
        : chainKey === "litecoin"
          ? resolveLtcPathByAddress(paymentAddress)
          : assetOps[chainKey];
    ...
    let signApiName = "";
    let responseData;
    const defaultTransactionFailureMessage = () =>
      t
        ? t("The transaction could not be completed. Please try again.")
        : "The transaction could not be completed. Please try again.";
    const normalizeFailureMessage = (message) => {
      const reasonKey = getTransactionFailureReasonKey(message);
      if (reasonKey) {
        return t ? t(reasonKey) : reasonKey;
      }
      return message;
    };
    const notifySignFailure = async (message) => {
      const displayMessage = normalizeFailureMessage(message);
      try {
        if (typeof setCheckStatusModalVisible === "function") {
          setCheckStatusModalVisible(false);
        }
        if (typeof setErrorModalMessage === "function") {
          setErrorModalMessage(displayMessage);
        }
    ...
    const isEvmForPost = families?.evm?.includes?.(postChain);
    const isEvmTokenTransferForPost =
      isEvmForPost && normalizedProtocol && normalizedProtocol !== "native";
    const isSuiTokenTransferForPost =
      postChain === "sui" && suiTransferType === "token";
    const normalizedTokenAddressForPost = String(contractAddress || "").trim();
    ...
    const postData = {
      ...
      extJson: {
        ...((postChain === "sui" ? suiTransferType : normalizedProtocol)
          ...
          : {}),
        ...((isEvmTokenTransferForPost || isSuiTokenTransferForPost) &&
          normalizedTokenAddressForPost
          ? {
              tokenAddress: normalizedTokenAddressForPost,
              tokenAmount: `${amount}`,
            }
          : {}),
        feePreference,
    ...
    signApiName = "accountAPI.getSignParam";
    let walletParamsResponse;
    try {
      walletParamsResponse = await fetch(accountAPI.getSignParam, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      console.log("Failed to get nonce and gasPrice:", error?.message || error);
      await notifySignFailure(defaultTransactionFailureMessage());
      return;
    }
    ...
    let walletParamsData;
    try {
      walletParamsData = await walletParamsResponse.json();
    } catch (error) {
      console.log("Failed to parse getSignParam response:", error?.message || error);
      await notifySignFailure(defaultTransactionFailureMessage());
      return;
    }
    console.log(
    ...
    if (!isApiSuccess(walletParamsData)) {
      const rawServerMessage = String(walletParamsData?.msg || "").trim();
      const serverMessage =
        rawServerMessage && rawServerMessage.toLowerCase() !== "success"
          ? rawServerMessage
          : defaultTransactionFailureMessage();
      console.log("getSignParam returned failure, terminating signing flow:", {
        chain: postChain,
        code: walletParamsData?.code,
        msg: walletParamsData?.msg,
      });
      await notifySignFailure(serverMessage);
      return;
    }
    ...
    if (families?.evm?.includes?.(postChain)) {
      ...
      const resolvedCosmosFee = resolveCosmosFeeAmount({
        feeAmountRaw: data.feeAmount,
        gasPrice,
        gasLimit,
        selectedFeeTab,
        recommendedFee,
      ...
      effectiveFeeAmountStr = resolvedCosmosFee.feeAmountStr;
      effectiveFeeAmount = resolvedCosmosFee.feeAmount;
      console.log("Cosmos fee resolved:", {
        chainKey,
        selectedFeeTab,
        gasPrice,
        gasLimit,
        feeAmount: effectiveFeeAmountStr,
        feeSource: resolvedCosmosFee.feeSource,
        candidates: resolvedCosmosFee.candidates,
      });
      ...
      const normalizedSuiObjects = Array.isArray(suiObjectsRaw)
        ...
        : [];
      if (!normalizedSuiObjects.length) {
        console.log(
          "The server returned no Sui gas objects and the signing process is terminated:",
          {
      ...
      const backendTokenObjects = Array.isArray(tokenObjectsRaw)
        ? tokenObjectsRaw
            .filter((o) => o?.objectId)
            .map((o) => ({
              objectId: o.objectId,
        ...
        : [];
      const gasObjectIds = new Set(normalizedSuiObjects.map((o) => o.objectId));
      ...
      suiObjects = normalizedSuiObjects;
      tokenObjects = dedupeByObjectId(backendTokenObjects);
    ...
    const guessOutputTypeFromAddress = (addr) => {
      ...
      if (a.startsWith("bc1") || a.startsWith("tb1") || a.startsWith("ltc1"))
        return "P2WPKH";
      if (
        raw.startsWith("3") ||
        raw.startsWith("2") ||
    ...
    const estimateBtcVSize = (utxos = [], fromAddr, toAddr) => {
      try {
        ...
        for (const u of utxos) {
          const it = guessInputType({ ...u, address: u?.address || fromAddr });
    ...
    if (chainMethod === "evm") {
      ...
      const serverFeeRate = Number(feeRate);
      let feeRateToUse = serverFeeRate;
      if (isBtcFamily && selectedFeeAmountCoin > 0) {
        ...
        const derived = Math.max(1, Math.ceil(satTotal / Math.max(estVb, 1)));
        const relaySafeFeeRate = Math.max(3, derived);
        feeRateToUse = Math.max(serverFeeRate || 0, relaySafeFeeRate);
    ...
    if (chainMethod === "tron") {
      ...
      const { amountDecimals, overrideDecimals } = resolveTronSigningDecimals({
        tokenDecimals,
      });
      const valueBaseUnits = String(
        Math.max(
          0,
          Math.round(Number(amount) * Math.pow(10, amountDecimals)),
        ),
      );
      const feeSun = resolveTronFeeSun(transactionFee);
    ...
    if (responseData?.data?.data) {
      ...
      const signAddrFormat = destAddrFormat;
      const signMessage =
        bleCmd.sign(chainKey, path, signPayloadRaw, signAddrFormat) + "\r\n";
      console.log(
        `App sends the payload waiting for device signature:\nchain=${chainKey}\npath=${path}\naddr_format=${signAddrFormat}\npayload=${signPayloadPretty}`,
      );
      ...
      try {
        ...
        await notifySignFailure(
          t
            ? t("Sign failed. Please try again.")
