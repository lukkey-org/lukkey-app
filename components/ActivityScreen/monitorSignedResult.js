...
const isCosmosFamilyChain = (chain) =>
  families.cosmos.includes(String(chain || "").trim().toLowerCase());
...
const createMonitorSignedResult = ({
  ...
  const showBroadcastModal = (msg, txHash) => {
    try {
      ...
      const failureReasonKey = getTransactionFailureReasonKey(msg);
      const failureTitle = failureReasonKey
        ? t("Transaction not sent")
        : t("Transaction Failed");
      const failureSubtitle = failureReasonKey
        ? t(failureReasonKey)
        : msg
          ? String(msg)
      ...
      if (success) {
        ...
        try {
          setVerificationStatus && setVerificationStatus("txSent");
        } catch {}
      } else {
        setModalStatus({
          title: failureTitle,
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (statusSuccess && shouldProcessStatus) {
          ...
          try {
            setVerificationStatus && setVerificationStatus("txSuccess");
          } catch {}
            ...
            while (true) {
              ...
              setTimeout(async () => {
                try {
                  ...
                  } else if (isCosmosFamilyChain(chainNameForFlow)) {
                    try {
                      const ctxStr = await AsyncStorage.getItem(
                      ...
                      const cosmosBroadcastPayload = {
                        chain: ctx.chain || chainNameForFlow,
                        from: ctx.from || selectedAddress,
                        to: ctx.to || "",
                        amount: Number(ctx.amount ?? 0),
