...
const getBalanceSymbolCandidates = (card) => {
  const chain = normalizeLower(getAssetChainFullName(card));
  const shortName = normalizeLower(getAssetSymbol(card));
...
const isLikelyNativeCard = (card) => {
  ...
  const coinType = normalizeLower(getAssetType(card));
  if (coinType === "native") return true;

  const shortName = normalizeLower(getAssetSymbol(card));
  const chainShortName = normalizeLower(getAssetChainShortName(card));
...
const findMatchedAsset = (card, assets) => {
  ...
  const cardContract = normalizeLower(
  ...
  const cardName = normalizeLower(getAssetDisplayName(card));
  if (cardName) {
...
const isTokenCard = (card) => {
  ...
  const coinType = normalizeLower(getAssetType(card));
  return Boolean(coinType && coinType !== "native");
};
...
const shouldRetrySuspiciousNativeZeros = (cards, accounts) => {
  ...
  for (const card of nativeCards) {
    const chainKey = normalizeChainKey(getAssetChainFullName(card));
...
const getCardMergeKey = (card) => {
  const chain = normalizeChainKey(getAssetChainFullName(card));
  const address = normalizeAddressKey(card?.address, chain);
  const contract = normalizeLower(
    card?.contractAddress || card?.contract_address || card?.tokenContractAddress,
  );
  const tokenId = normalizeLower(getNftMintValue(card));
  const symbol = normalizeLower(getAssetSymbol(card));
  const name = normalizeLower(getAssetDisplayName(card));
...
export const fetchPriceChanges = async (
  ...
  if (cryptoCards.length === 0) {
    setRefreshing(false);
    return; // Do not request when there is no card
  }
  ...
  try {
    ...
    if (data.code === 0 && data.data) {
      ...
      (cryptoCards || []).forEach((card) => {
        const cardSymbol = String(getAssetSymbol(card) || "")
          .trim()
          .toUpperCase();
...
export const fetchWalletBalance = async (
  ...
  try {
    ...
    for (const c of cards) {
      const chain = String(getAssetChainFullName(c) || "").trim();
            ...
            if (acc && Array.isArray(acc.assets)) {
              ...
              if (!asset) {
                ...
                devLog("[AssetsDataFetcher] asset match found ->", {
                  chain: chainKey,
                  address: addrKey,
                  cardSymbol: getAssetSymbol(card),
              ...
              if (asset) {
                ...
                nextBalance = "0.0";
                devLog("[AssetsDataFetcher] token asset missing from balance response, using 0.0 ->", {
                  chain: chainKey,
                  address: addrKey,
                  cardSymbol: getAssetSymbol(card),
                });
