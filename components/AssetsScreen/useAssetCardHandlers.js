  ...
  const handleCardPress = React.useCallback(
    (cryptoName, cryptoChain, index) => {
      ...
      const crypto = cryptoCards.find(
        (card) =>
          getAssetDisplayName(card) === cryptoName && getAssetChainFullName(card) === cryptoChain,
      );
      if (!crypto) {
      ...
      const commitSelection = () => {
        ...
        setSelectedCardChainShortName(getAssetChainShortName(crypto) || "");
      ...
      setCardDetailContentVisible(true);
