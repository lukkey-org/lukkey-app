...
const AssetsWalletPage = ({
  ...
  const getStableId = React.useCallback(
    (card) => {
      const chain =
        getAssetChainFullName(card) || "chain";
      ...
      const symbol = getAssetSymbol(card) || "";
      const name = getAssetDisplayName(card) || "card";
  ...
  // Only one "network fee token" is marked per chain (type=native is preferred).
  ...
  const shouldShowCardDataSkeleton =
    showInitialCardSkeleton || (!hasRenderableCards && showHeaderSkeletonState);
  ...
  const swapOrderTags = React.useCallback(
    (movingId, targetId) => {
          ...
          if (Number.isFinite(currentLayoutY)) {
            ...
            if (movingDown && currentIndex < ordered.length - 1) {
              ...
              if (
                ...
                targetY <=
                  (currentLayoutY + prevLayoutY) / 2 - swapHysteresisUp
              ) {
                const now = Date.now();
                const lastSwap = lastSwapRef.current || {};
                if (
  ...
  const getCardKey = React.useCallback(
    (card, index) => {
      const chain =
        getAssetChainFullName(card) || "chain";
      const address = getCardIdentityAddress(card);
      const name = getAssetSymbol(card) || getAssetDisplayName(card) || "card";
      return `${chain}:${address || name}:${index}`;
    },
    [getCardIdentityAddress],
  );

  const getEntryAnim = React.useCallback(
    (key) => {
      if (!entryAnimMapRef.current.has(key)) {
        const shouldShowImmediately =
          entryAnimatedKeysRef.current.has(key) ||
          modalVisible ||
          isClosing ||
          isOpening;
        entryAnimMapRef.current.set(
          key,
          new Animated.Value(shouldShowImmediately ? 1 : 0),
        );
      }
        ...
        >
          ...
          >
            ...
            >
              {!hideWalletContent &&
                ...
                (!modalVisible || isClosing) && (
                  <TotalBalanceHeader
                    ...
                    isBalanceSyncing={
                      refreshing || isBalanceSyncing
                    }
                    isPriceLoading={
                      refreshing || (showInitialCardSkeleton && isPriceLoading)
                    }
              ...
              {!hideWalletContent && showCardListSkeleton
                  ...
                  ? orderedCards.map((item, uiIndex) => {
                    ...
                    const stableIndex =
                      Number.isInteger(dataIndex) && dataIndex >= 0
                        ? dataIndex
                        : uiIndex;
                    const isBlackText = [""].includes(getAssetSymbol(card));
                    const priceChange =
                      priceChanges[getAssetSymbol(card)]?.priceChange || "0";
                    const percentageChange =
                      priceChanges[getAssetSymbol(card)]?.percentageChange || "0";
                    if (
                    ...
                    const chainKey = String(getAssetChainFullName(card) || "")
                      .trim()
                      .toLowerCase();
                    const symbolKey = String(getAssetSymbol(card) || "")
                      .trim()
                      .toLowerCase();
                    ...
                    return (
                      <CardItem
                        ...
                        showExpandedCardContent={showExpandedCardContent}
