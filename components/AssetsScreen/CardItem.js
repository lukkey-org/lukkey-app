...
import { isBchCard } from "../../utils/bchAddress";
...
const buildAddressTypeOption = ({
  ...
  const formattedBalance =
    safeBalance === 0
      ? "0"
      : formatCryptoBalanceDisplay(safeBalance, {
          symbol,
          context: "CardAddressTypeSelect.balance",
          compactLarge: true,
        });
  ...
  return {
    ...
    balanceValue: safeBalance,
    balanceText: compactIfTooLong(formattedBalance, safeBalance),
    balanceFiatText: getTypedFiatText?.(safeBalance, symbol),
  };
};

const buildTotalBalanceSummary = (options, symbol, getTypedFiatText) => {
  const totalBalanceValue = (Array.isArray(options) ? options : []).reduce(
    (sum, option) => {
      const value = Number(option?.balanceValue);
  ...
  const formattedBalance =
    totalBalanceValue === 0
      ? "0"
      : formatCryptoBalanceDisplay(totalBalanceValue, {
          symbol,
          context: "CardAddressTypeSelect.total",
          compactLarge: true,
        });
  return {
    totalBalanceValue,
    totalBalanceText: compactIfTooLong(formattedBalance, totalBalanceValue),
    totalFiatText: getTypedFiatText?.(totalBalanceValue, symbol) || "",
  };
};

const getCompactAddressTypeLabel = (label) =>
  String(label || "")
    .split("/")
    [0].trim();

const buildCardAddressTypeConfig = (card, { getTypedFiatText } = {}) => {
  const safeAddress = String(card?.address || "").trim();
  const symbol = String(getAssetSymbol(card) || "").trim().toUpperCase();

  if (isBtcCard(card)) {
    const activeType = normalizeBtcAddressType(card?.btcAddressType);
    const args = [
      safeAddress,
      card?.btcLegacyAddr,
      card?.btcNestedSegwitAddr,
      card?.btcNativeSegwitAddr,
      card?.btcTaprootAddr,
    ];
    const options = [
      buildAddressTypeOption({
        key: BTC_ADDRESS_TYPES.LEGACY,
        label: "Legacy / P2PKH",
        preview: resolveBtcAddressByType(BTC_ADDRESS_TYPES.LEGACY, ...args),
        balanceValue: card?.btcLegacyBalance,
        symbol,
        getTypedFiatText,
      }),
      buildAddressTypeOption({
        ...
        preview: resolveBtcAddressByType(BTC_ADDRESS_TYPES.NESTED_SEGWIT, ...args),
        balanceValue: card?.btcNestedSegwitBalance,
        symbol,
        getTypedFiatText,
      }),
      buildAddressTypeOption({
        ...
        preview: resolveBtcAddressByType(BTC_ADDRESS_TYPES.NATIVE_SEGWIT, ...args),
        balanceValue: card?.btcNativeSegwitBalance,
        symbol,
        getTypedFiatText,
      }),
      buildAddressTypeOption({
        ...
        preview: resolveBtcAddressByType(BTC_ADDRESS_TYPES.TAPROOT, ...args),
        balanceValue: card?.btcTaprootBalance,
        symbol,
        getTypedFiatText,
      }),
    ...
    return {
      activeType,
      options,
      ...buildTotalBalanceSummary(options, symbol, getTypedFiatText),
      onSelectProp: "onSwitchBtcAddressType",
    };
  }

  if (isBchCard(card)) {
    const activeType = normalizeBchAddressType(card?.bchAddressType);
    const cashAddr = resolveBchAddressByType(
      BCH_ADDRESS_TYPES.CASHADDR,
      safeAddress,
      card?.bchCashAddr,
      card?.bchLegacyAddr,
    );
    const displayCashAddr = cashAddr
      ? cashAddr.includes(":")
        ? cashAddr
        : `bitcoincash:${cashAddr}`
      : "";
    /*
     * Temporary production restriction: BCH asset cards only expose CashAddr.
     * Keep the legacy selector setup commented so it can be restored quickly
     * after legacy BCH receive selection is re-enabled.
     *
     * const legacyAddr = resolveBchAddressByType(
     *   BCH_ADDRESS_TYPES.LEGACY,
     *   safeAddress,
     *   card?.bchCashAddr,
     *   card?.bchLegacyAddr,
     * );
     * const legacyLabel = String(legacyAddr || "").startsWith("3")
     *   ? "Legacy format / P2SH"
     *   : "Legacy format / P2PKH";
     */
    const options = [
      buildAddressTypeOption({
        ...
        label: "CashAddr format / P2PKH",
        preview: displayCashAddr,
        balanceValue: card?.bchCashaddrBalance,
        symbol,
        getTypedFiatText,
      }),
      /*
       * Temporary production restriction: legacy BCH is intentionally hidden.
       * Restore this option with the legacy variables above when legacy
       * receive selection is re-enabled.
       *
       * buildAddressTypeOption({
       *   key: BCH_ADDRESS_TYPES.LEGACY,
       *   label: legacyLabel,
       *   preview: legacyAddr,
       *   balanceValue: card?.bchLegacyBalance,
       *   symbol,
       *   getTypedFiatText,
       * }),
       */
    ...
    const resolvedType =
      isBchCashAddr(displayCashAddr) || cashAddr
        ? BCH_ADDRESS_TYPES.CASHADDR
        : activeType;
    return {
      ...
      options,
      ...buildTotalBalanceSummary(options, symbol, getTypedFiatText),
      onSelectProp: "onSwitchBchAddressType",
    };
  }

  if (isLtcCard(card)) {
    const activeType = normalizeLtcAddressType(card?.ltcAddressType);
    const args = [
      safeAddress,
      card?.ltcLegacyAddr,
      card?.ltcNestedSegwitAddr,
      card?.ltcNativeSegwitAddr,
    ];
    const options = [
      buildAddressTypeOption({
        key: LTC_ADDRESS_TYPES.LEGACY,
        label: "Legacy / P2PKH",
        preview: resolveLtcAddressByType(LTC_ADDRESS_TYPES.LEGACY, ...args),
        balanceValue: card?.ltcLegacyBalance,
        symbol,
        getTypedFiatText,
      }),
      buildAddressTypeOption({
        ...
        label: "Nested SegWit / P2SH",
        preview: resolveLtcAddressByType(LTC_ADDRESS_TYPES.NESTED_SEGWIT, ...args),
        balanceValue: card?.ltcNestedSegwitBalance,
        symbol,
        getTypedFiatText,
      }),
      buildAddressTypeOption({
        ...
        label: "Native SegWit / Bech32",
        preview: resolveLtcAddressByType(LTC_ADDRESS_TYPES.NATIVE_SEGWIT, ...args),
        balanceValue: card?.ltcNativeSegwitBalance,
        symbol,
        getTypedFiatText,
      }),
    ].filter((option) => option.preview);
    return {
      ...
      options,
      ...buildTotalBalanceSummary(options, symbol, getTypedFiatText),
      onSelectProp: "onSwitchLtcAddressType",
    };
  }
...
const CardAddressTypeSelect = ({
  ...
  const activeOption =
    options.find((option) => option.key === config?.activeType) || options[0];
  if (!activeOption || options.length < 2 || typeof onSelect !== "function") {
    return null;
  }

  const textColor = isBlackText ? "#333" : "#eee";
  const panelBg = isDarkMode ? "#3A3838" : "#FFFFFF";
  const panelBorder = isDarkMode ? "transparent" : "#E5E5EC";
  const optionText = isDarkMode ? "#F1F1F3" : "#333";
  ...
  const menuLeft = Math.max(12, (windowWidth - menuWidth) / 2);
  const menuTop = menuAnchor ? menuAnchor.y + menuAnchor.height + 6 : 0;
  const openMenu = () => {
  ...
  return (
    <View style={styles.cardAddressTypeSelect}>
      <TouchableOpacity
        ref={buttonRef}
        activeOpacity={0.82}
        ...
        style={styles.cardAddressTypeButton}
      >
        <Text
          numberOfLines={1}
          style={[styles.cardAddressTypeButtonText, { color: textColor }]}
        >
          {getCompactAddressTypeLabel(activeOption.label)}
        </Text>
        <Icon
          name={open ? "keyboard-arrow-up" : "keyboard-arrow-down"}
          size={16}
          color={textColor}
          style={styles.cardAddressTypeIcon}
        />
      </TouchableOpacity>
      {open ? (
        <Modal
          ...
          <View
            style={[
              styles.cardAddressTypeMenu,
              {
                ...
                shadowColor: isDarkMode ? "#000" : "#8E8E99",
              },
            ]}
          >
            {options.map((option) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.cardAddressTypeMenuItem,
                    selected && { backgroundColor: selectedBg },
                  ]}
                >
                  <View style={styles.cardAddressTypeMenuRow}>
                    <View style={styles.cardAddressTypeMenuLabelCol}>
                      <Text
                        numberOfLines={1}
                        style={[
                          styles.cardAddressTypeMenuText,
                          {
                            color: optionText,
                      ...
                      {option.preview ? (
                        <Text
                          numberOfLines={1}
                          style={[
                            styles.cardAddressTypeMenuPreview,
                            { color: isDarkMode ? "#BDBDC5" : "#7A7A85" },
                          ]}
                        >
                          {option.preview}
                        </Text>
                    ...
                    {option.balanceText ? (
                      <View style={styles.cardAddressTypeMenuBalanceCol}>
                        <View style={styles.cardAddressTypeMenuBalanceRow}>
                          <Text
                            numberOfLines={1}
                            style={[
                              styles.cardAddressTypeMenuBalanceText,
                              {
                                color: balanceText,
                          ...
                          {cryptoIcon ? (
                            <Image
                              source={cryptoIcon}
                              style={styles.cardAddressTypeMenuBalanceIcon}
                            />
                          ) : null}
                        </View>
                        {option.balanceFiatText ? (
                          <Text
                            numberOfLines={1}
                            style={[
                              styles.cardAddressTypeMenuFiatText,
                              { color: balanceMuted },
                            ]}
                          >
                            {option.balanceFiatText}
            ...
            <View
              style={[
                styles.cardAddressTypeMenuDivider,
                {
            ...
            />
            <View style={styles.cardAddressTypeMenuTotalRow}>
              <Text
                numberOfLines={1}
                style={[
                  styles.cardAddressTypeMenuTotalLabel,
                  { color: balanceMuted },
                ]}
              >
                {totalLabel}
 ...
 * @param {boolean} props.showExpandedCardContent - whether to show expanded information on the card surface
...
const CardItem = ({
  ...
  receiveMultiAddressEnabled = false,
  ...
  const getDisplayName = (name) => {
    ...
    if (
      name === "USD Coin" &&
      String(getAssetChainFullName(card) || "").toLowerCase() === "binance" &&
      String(getAssetSymbol(card) || "").toUpperCase() === "USDC"
    ) {
      return "USDC";
    }
  ...
  const resolvedChainIcon = React.useMemo(
    () => resolveChainIcon(getAssetChainFullName(card)),
    [card],
  );
  const getCardAddressTypeFiatText = React.useCallback(
    (amountValue, symbol) => {
      const converted =
    ...
    [currencyUnit, getConvertedBalance],
  );
  const addressTypeConfig = React.useMemo(
    () =>
      buildCardAddressTypeConfig(card, {
        getTypedFiatText: getCardAddressTypeFiatText,
      }),
    [card, getCardAddressTypeFiatText],
  );
  const handleCardAddressTypeSelect = React.useCallback(
    (nextType) => {
      if (addressTypeConfig?.onSelectProp === "onSwitchBchAddressType") {
        onSwitchBchAddressType?.(nextType);
      } else if (addressTypeConfig?.onSelectProp === "onSwitchBtcAddressType") {
        onSwitchBtcAddressType?.(nextType);
      } else if (addressTypeConfig?.onSelectProp === "onSwitchLtcAddressType") {
        onSwitchLtcAddressType?.(nextType);
      }
    },
    [
      addressTypeConfig?.onSelectProp,
      onSwitchBchAddressType,
      onSwitchBtcAddressType,
      onSwitchLtcAddressType,
    ],
  );
  const showCardAddressTypeSelect =
    receiveMultiAddressEnabled &&
    Array.isArray(addressTypeConfig?.options) &&
    addressTypeConfig.options.length > 1 &&
    showExpandedCardContent &&
    !isClosing &&
    isCardExpanded &&
    selectedCardIndex === index;
  ...
  }, [runtimeBalance, getAssetSymbol(card), formatBalance]);
  ...
  const handlePress = () => {
    ...
    if (!modalVisible) {
      ...
      const opened = handleCardPress(getAssetDisplayName(card), getAssetChainFullName(card), index);
    ...
    <>
      ...
      >
        <TouchableHighlight
          ...
          activeOpacity={1}
          key={`${getAssetSymbol(card)}_${index}`}
                ...
                <View style={styles.cardHeaderGroup}>
                  ...
                  <View
                    style={[
                      ...
                      showCardAddressTypeSelect &&
                        styles.cardTitleGroupWithAddressType,
                    ]}
                  >
                      ...
                      {["cardName", "chainText"].map((textStyle, i) =>
                        i === 0 ? (
                          <View key={i} style={styles.cardTitleRow}>
                            ...
                            >
                              {getDisplayName(getAssetDisplayName(card))}
                            </Text>
                            {showGasFeeIcon ? (
                              <FontAwesome6
                                name="gas-pump"
                          ...
                          <View key={i} style={styles.cardChainRow}>
                            <View
                              style={[
                            ...
                            >
                              ...
                              >
                                {getAssetChainFullName(card)
                                  ? displayChainName(getAssetChainFullName(card))
                                : ""}
                              </Text>
                            </View>
                            {showCardAddressTypeSelect ? (
                              <CardAddressTypeSelect
                                ...
                                cryptoIcon={resolvedAssetIcon}
                                isBlackText={isBlackText}
                                isDarkMode={isDarkMode}
                                onSelect={handleCardAddressTypeSelect}
                          ...
                          {isDataLoading ? (
                            ...
                            >
                              {hideNumbers
                                ...
                                : `${formattedBalance}  ${getAssetSymbol(card)}`}
                            </Text>
                          )}
                        ...
                        >
                          {["carBalCen", "balShortNameCtr"].map((styleKey, i) => {
                            ...
                            const displayValue = Number.isFinite(numericValue)
                              ...
                              : (() => {
                                  console.log(
                                    `[Vault][CardItem] modal display value is non-finite, using 0.00 -- card=${getAssetSymbol(card)}, index=${i}, raw=${rawValue}, balance=${runtimeBalance}`,
                                  );
                                  return "0.00";
                                })();
                            const unitLabel =
                              i === 0 ? getAssetSymbol(card) : currencyUnit;
...
export default React.memo(CardItem, (prev, next) => {
  ...
  return (
    ...
    prev.showExpandedCardContent === next.showExpandedCardContent &&
    ...
    prev.card?.bchAddressType === next.card?.bchAddressType &&
    prev.card?.btcAddressType === next.card?.btcAddressType &&
    prev.card?.ltcAddressType === next.card?.ltcAddressType &&
    getAssetDisplayName(prev.card) === getAssetDisplayName(next.card) &&
    getAssetSymbol(prev.card) === getAssetSymbol(next.card) &&
  getAssetChainFullName(prev.card) === getAssetChainFullName(next.card) &&
    ...
    prev.receiveMultiAddressEnabled === next.receiveMultiAddressEnabled &&
