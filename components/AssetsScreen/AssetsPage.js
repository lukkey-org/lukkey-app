...
const AssetsPage = ({
  ...
  const nftQueryTargets = useMemo(() => {
    ...
    for (const c of cards) {
      const chain = getAssetChainFullName(c);
      ...
      for (const addr of addresses) {
        ...
        if (!unique.has(key)) {
          unique.set(key, {
            ...
            addrFormat: isBtcCard(c) ? getBtcAddressType(normalizedAddr) : "",
          });
        }
      }
    }
    ...
    runFetchPromise = (async () => {
      try {
        const targetMap = new Map(
          targets.map(({ chain, address, addrFormat }) => [
            `${normalizeChainName(chain)}:${normalizeLower(address)}`,
            { chain, address, addrFormat },
          ]),
        );
        ...
        const combined = (ok ? getNftListFromResponse(json) : []).map((item) => {
          ...
          return {
            ...
            sourceQueryAddress: matchedTarget?.address || "",
            sourceAddrFormat: matchedTarget?.addrFormat || "",
  ...
  const feeCardForNFT = React.useMemo(() => {
    ...
    return (
      cards.find(
        (c) =>
          getAssetChainFullName(c)?.toLowerCase?.() === chainKey &&
          String(getAssetSymbol(c) || "").trim().toLowerCase() === feeSymbol,
      ) || null
    );
  ...
  const handleSendDigitalFromPreview = async (opts = {}) => {
    ...
    const paymentAddress = paymentAddressForNFT || fromCard?.address || "";
      ...
      >
        {selectedChain === "All" ? (
          ...
          (() => {
            ...
            return chainSelectorCards
              .filter((card) => {
                const chainName = getAssetChainFullName(card);
                if (
                  selectedChain === chainName &&
                  resolveChainIcon(getAssetChainFullName(card)) &&
                  !uniqueChainIcons.has(chainName)
                ) {
                  uniqueChainIcons.add(chainName);
                  return true;
                }
                return false;
              })
              .map((card, index) => (
                <Image
                  key={`${getAssetChainFullName(card)}-${index}`}
                  source={resolveChainIcon(getAssetChainFullName(card))}
        ...
        <Text style={{ color: isDarkMode ? "#FFFFFF" : "#000000" }}>
          {selectedChain === "All"
            ? t("All Chains")
            : (() => {
                const card = chainSelectorCards.find((card) => {
                  const chainName = getAssetChainFullName(card);
                  return chainName === selectedChain;
                });
                const name = getAssetChainFullName(card);
                return displayChainName(name);
              })()}
        </Text>
      </TouchableOpacity>
    );
  ...
  const galleryFilterCards = useMemo(() => {
    ...
    (Array.isArray(cryptoCards) ? cryptoCards : []).forEach((card) => {
      const chainName = normalizeChainName(getAssetChainFullName(card));
    ...
    list.forEach((nft) => {
      ...
      const matchedCard = cardMap.get(`name:${nftChainName}`);
      const key = nftChainName || normalizeChainName(getAssetChainFullName(matchedCard));
      if (!key || optionsMap.has(key)) return;

      const rawName =
        nft?.queryChainName ||
        getAssetChainFullName(matchedCard) ||
        getAssetDisplayName(matchedCard) ||
        key;

      optionsMap.set(key, {
        chainFullName: key,
        queryChainName: displayChainName(rawName),
        chainIcon: resolveChainIcon(
          getAssetChainFullName(matchedCard) || nftChainName,
        ),
      });
    });
  ...
  const filteredNftData = useMemo(() => {
    ...
    const matchedChainNames = new Set(
      galleryFilterCards
        ...
        .map((card) => normalizeChainName(getAssetChainFullName(card))),
    );

    const filteredList = list.filter((nft) => {
      const nftChainName = normalizeChainName(nft?.queryChainName);
      return nftChainName === selectedChain || matchedChainNames.has(nftChainName);
    });
  ...
  return selectedView === "wallet" ? (
    <AssetsWalletPage
      ...
      hideWalletContent={!!props.hideWalletContent}
      isClosing={isClosing}
      ...
      cardDetailContentVisible={props.cardDetailContentVisible}
      ...
      receiveMultiAddressEnabled={props.receiveMultiAddressEnabled}
      onSwitchBchAddressType={props.onSwitchBchAddressType}
      onSwitchBtcAddressType={props.onSwitchBtcAddressType}
      onSwitchLtcAddressType={props.onSwitchLtcAddressType}
