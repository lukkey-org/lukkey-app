...
const TabView = ({
  ...
  const matchesSelectedAddress = React.useCallback(
    (candidateAddress, crypto = selectedCrypto) => {
      const chain = String(getAssetChainFullName(crypto) || "").trim();
  ...
  useEffect(() => {
    const chain = String(getAssetChainFullName(selectedCrypto) || "").trim();
    ...
    latestChainAddrRef.current =
      chain && addresses.length > 0
        ? addresses
            ...
            .join(",")
        : "";
  }, [getAssetChainFullName(selectedCrypto), selectedCrypto?.address]);
    ...
    switch (activeTab) {
      ...
      case "Prices": {
        ...
        const priceIconSource =
          ...
          resolveChainIcon(getAssetChainFullName(selectedCrypto)) ||
        ...
        return (
          <View style={VaultScreenStyle.priceContainer}>
            <PriceChartCom
              ...
              debugPriceLabel={`${getAssetSymbol(selectedCrypto) || "Unknown"}-${
                currencyUnit || "USD"
              }`}
