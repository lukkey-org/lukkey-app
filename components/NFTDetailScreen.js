...
const SkeletonImage = ({ source, style, VaultScreenStyle }) => {
  ...
  const imageTranslateY = React.useState(new Animated.Value(14))[0];
...
const NFTDetailScreen = () => {
  ...
  const mediaEntryTranslateY = useRef(new Animated.Value(28)).current;
  const mediaEntryOpacity = useRef(new Animated.Value(0)).current;
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.timing(mediaEntryTranslateY, {
        toValue: 0,
        duration: 360,
        useNativeDriver: true,
      }),
      Animated.timing(mediaEntryOpacity, {
        toValue: 1,
        duration: 260,
        useNativeDriver: true,
      }),
    ]).start();
  }, [mediaEntryOpacity, mediaEntryTranslateY]);
          ...
          {selectedNFT ? (
            <Animated.View
              style={[
                styles.section,
                {
                  opacity: mediaEntryOpacity,
                  transform: [{ translateY: mediaEntryTranslateY }],
                },
              ]}
            >
              {selectedNFT.logoUrl ? (
