...
const ActivityLogScreen = () => {
  ...
  const matchCrypto = (item, tx) => {
    ...
    return areAddressesEquivalent(getAssetChainFullName(item), address, tx?.address);
  };
  ...
  const transactionChainCards = useMemo(() => {
    ...
    ActivityLog.filter(
      ...
      const card = initialAdditionalCryptos.find(
        (item) =>
          String(getAssetChainFullName(item) || "")
            .trim()
            .toLowerCase() === chainNameLc,
      );
      const chainKey = getAssetChainFullName(card);
      if (card && chainKey && !map.has(chainKey)) {
        map.set(chainKey, card);
      }
    });
    ...
    selectedChain === "All"
      ...
      : ActivityLog.filter((tx) => {
          ...
          const hit = initialAdditionalCryptos.find(
            (item) =>
              getAssetChainFullName(item) === selectedChain &&
              String(getAssetChainFullName(item) || "")
                .trim()
                .toLowerCase() === chainNameLc,
          );
              ...
              >
                {selectedChain === "All" ? (
                  ...
                  transactionChainCards
                    .filter(
                      (card) => getAssetChainFullName(card) === selectedChain,
                    )
                    .map((card, index) => (
                      <Image
                ...
                >
                  {selectedChain === "All"
                    ? t("All Chains")
                    : (() => {
                        const card = transactionChainCards.find(
                          (card) => getAssetChainFullName(card) === selectedChain,
                        );
