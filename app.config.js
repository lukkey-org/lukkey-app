...
function httpsOriginFromHost(host) {
  const trimmedHost = String(host || "").trim();
  return trimmedHost ? ["https:/", trimmedHost].join("/") : null;
}
...
function loadPrivateRuntimeConfig() {
  ...
  return {
    ...
    marketOrigin: httpsOriginFromHost(serviceHostsObject?.market),
    ...
    fileOrigin: httpsOriginFromHost(serviceHostsObject?.file),
    ...
    ["firmware" + "List" + "Token"]:
      firmwareApiObject?.["lvgl" + "List" + "Token"] || null,
    firmwareMd5Base,
...
export default {
  expo: {
    ...
    //Key points
    plugins: [
      "./plugins/withProductionExpoDevClientExclusion",
      "expo-secure-store",
      "expo-localization",
      [
        "expo-notifications",
